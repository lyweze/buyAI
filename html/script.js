//Фон
var m = 0;
const startg = (entriesg, down) => {
    entriesg.forEach(entriesg => {
        if(m==0){
            entriesg.target.classList.toggle("slide-in-from-g", entriesg.isIntersecting);
            if (entriesg.isIntersecting == true){
              m++;
            }
        }
    });
  ...
  const g = new IntersectionObserver(startg);
  const optionsg = { root: null, rootMargin: '0px', threshold: 1 }; 

  const elemg = document.querySelectorAll('.gradientt');
    elemg.forEach(d => {
    g.observe(d, optionsg);
    });
